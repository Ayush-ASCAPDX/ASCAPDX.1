...
async function configureSenderBandwidth(sender) {
  ...
  try {
    ...
    // Browsers may reject bitrate tuning on some platforms.
  }
}

async function applyRtcConfig() {
  try {
    const response = await authFetch("/api/rtc-config");
    ...
    rtcConfig = {
      ...DEFAULT_RTC_CONFIG,
      ...config,
      iceServers: [...config.iceServers, ...DEFAULT_RTC_CONFIG.iceServers]
    };
  } catch (_) {
...
async function tryIceRestart(reason = "network-change") {
  if (!peerConnection || !activePeer) return false;
  if (isRestartingIce) return false;
  ...
  setStatus(`Network issue (${reason}). Reconnecting... ${iceRestartAttempts}/${MAX_ICE_RESTART_ATTEMPTS}`);
...
function createPeerConnection() {
  ...
  pc.onconnectionstatechange = () => {
    ...
    if (pc.connectionState === "disconnected") {
      ...
      tryIceRestart("disconnected").catch(() => {});
      disconnectGraceTimer = setTimeout(() => {
        if (!peerConnection || peerConnection.connectionState === "connected") return;
        if (!callStatus.textContent.includes("ended")) setStatus("Call ended.");
    ...
    if (pc.connectionState === "failed") {
      clearDisconnectGraceTimer();
      tryIceRestart("failed").then((restarted) => {
        if (!restarted) {
          if (!callStatus.textContent.includes("ended")) setStatus("Call ended.");
          cleanupCall({ nextState: "closed", redirect: true });
        }
  ...
  pc.oniceconnectionstatechange = () => {
    ...
    if (pc.iceConnectionState === "failed") {
      tryIceRestart("ice-failed").catch(() => {});
    }
  };
...
async function startCall() {
  ...
  try {
    ...
    await Promise.all(senders.map((sender) => configureSenderBandwidth(sender)));
...
function setupSocket() {
  ...
  socket.on("video-offer", (payload) => {
    ...
    // During an active call, allow re-offers (renegotiation / ICE restart) from the same peer.
    if (peerConnection && activePeer && from === activePeer) {
      (async () => {
        try {
          ...
          socket.emit("video-answer", { to: from, answer, callId: callId || activeCallId });
        } catch (_) {
          // Ignore negotiation race failures; the next offer/answer cycle can recover.
        }
      })();
      return;
    }
